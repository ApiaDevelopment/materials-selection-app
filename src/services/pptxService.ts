...
async function fetchProjectData(
  ...
  // Filter to only line items with products
  const lineItemsWithProducts = lineItems.filter((item) => item.productId);

  // Fetch all related data for each line item
  const lineItemsWithDetails = await Promise.all(
    lineItemsWithProducts.map(async (lineItem) => {
      const product = await productService.getProduct(lineItem.productId!);
  ...
  lineItemsWithDetails.forEach((item) => {
...
async function generateProductSlide(
  ...
  // Status upper right
  const status = lineItem.status || "Selected";
  ...
  // Model and tier
  const modelTier = [];
  if (product.modelNumber) modelTier.push(`Model: ${product.modelNumber}`);
  if (product.tier) modelTier.push(`Tier: ${product.tier.toUpperCase()}`);

  if (modelTier.length > 0) {
    slide.addText(modelTier.join(" | "), {
      x: detailsX,
      y: yPos,
      w: detailsW,
      h: 0.3,
